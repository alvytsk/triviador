...
export function ChoiceEditor({
  ...
  choices: ChoiceWrite[];
  /** One message per choice, or `null` where that choice has none — same
   *  shape and same source (`questionWriteRequestSchema`'s `onSubmit`
   *  validator) as the prompt field's `field.state.meta.errors[0]?.message`
   *  a few lines up in `question-form.tsx`. Optional so every other caller
   *  (there are none today, but nothing requires one) doesn't have to
   *  thread four `null`s through. */
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {choices.map((choice, index) => (
        ...
        <div key={`choice-${index}`} className="flex flex-col gap-1">
          ...
          {/* No `maxLength` here on purpose — that would restate
           *  `choiceWriteSchema.text`'s server-owned bound a second time
           *  (the same house rule `preset-form.tsx` and `sign-in-form.tsx`
           *  already state in their own comments), and the two would drift
           *  the first time the contract moved. The bound is enforced
           *  here, visibly, by `questionWriteRequestSchema`'s own
           *  `onSubmit` validator instead. */}
          {errors?.[index] != null && (
            <p className="pl-7 text-[11px] font-medium text-bad">{errors[index]}</p>
          )}
        </div>
      ))}
    </div>

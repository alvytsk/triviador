...
- Every `datetime` reachable from an event must be aware and UTC. Pydantic
  ...
  this is enforced structurally by `_require_utc`, called on both `encode`
  and `decode`.
"""
...
def _require_utc(value: Any, path: str = "$") -> Any:
    ...
    if is_dataclass(value):
        updates = {
            f.name: _require_utc(getattr(value, f.name), f"{path}.{f.name}") for f in fields(value)
        }
    ...
    if isinstance(value, tuple):
        return tuple(_require_utc(item, f"{path}[{i}]") for i, item in enumerate(value))
    if isinstance(value, Mapping):
        return {key: _require_utc(item, f"{path}[{key!r}]") for key, item in value.items()}
    return value


def encode(event: GameEvent) -> tuple[str, int, dict[str, Any]]:
    wire_type = WIRE_NAMES[type(event)]
    normalized = _require_utc(event)
...
def decode(wire_type: str, schema_version: int, payload: Mapping[str, Any]) -> GameEvent:
    ...
    event = _adapter_for(cls).validate_python(upcast(dict(payload)))
    return _require_utc(event)  # type: ignore[no-any-return]
